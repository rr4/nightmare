...
function Nightmare(options) {
  ...
  this.queue((done) => {
    ...
    this.proc.on('close', (code) => {
      if(!self.ended){
        console.warn("---------------------------------")
        console.warn("---------- !self.ended ----------")
        console.warn("---------------------------------")
        // handleExit(code, self, noop);
        
        endInstance(self, noop, true)
...
function endInstance(instance, cb, forceKill) {
  ...
  if (instance.proc && instance.proc.connected) {
    console.warn("---------------------------------")
    console.warn("---------- "+instance.proc + instance.proc.connected +" ----------")
    console.warn("---------------------------------")
